...
console.log('Testing IBM Quantum Integration...');

// Import the quantum service directly
import { quantumService } from '../src/services/quantum/IBMQuantumService.js';
...
async function testQuantum() {
  ...
  const circuit = {
    ...
    qubits: 1,
    operations: [
      { gate: 'h', qubits: [0] },         // Hadamard gate
      { gate: 'measure', qubits: [0], cbits: [0] }  // Measurement
    ]
  };

  console.log('3. Running the quantum circuit...');
  try {
    const result = await quantumService.runCircuit(circuit, 10); // Only 10 shots for testing
